...
class RandomGrayscale(object):
    ...
    def __init__(self, gray_prob=0.5):
...
class RandomErasing(object):
    ...
    def __init__(self,
                 ...
                 mode='const',
                 fill_color=(100, 167, 160),
                 # fill_color=(random.randint(145,175),random.randint(102,132),random.randint(85,115)),
                 fill_std=None):
    ...
    def _fill_pixels(self, img, top, left, h, w):
        fill_color = (random.randint(85, 115), random.randint(102, 132), random.randint(145, 175))
        if self.mode == 'const':
            patch = np.empty((h, w, 3), dtype=np.uint8)
            patch[:, :] = np.array(fill_color, dtype=np.uint8)
